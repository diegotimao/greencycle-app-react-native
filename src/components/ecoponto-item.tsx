...
export default function EcopontoItem({ ecopontoItem }: ecopontoProps) {
  ...
  return (
    <Pressable 
      onPress={() => navigation()}
      className="w-full flex-row gap-4 p-6 justify-between items-center bg-white rounded-xl mb-3">
      ...
      <View className="-ml-12">
        ...
        <Text className="text-sm text-gray-600">Rua: {ecopontoItem.address.road} N: {ecopontoItem.address.number}</Text>
        <Text className="text-sm text-gray-600">{ecopontoItem.address.city} {ecopontoItem.address.country}</Text>
      </View>
